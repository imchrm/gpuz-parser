...
**Телефоны:** верхняя часть страницы маскирует номера (требует JS-клика). Полные номера доступны в FAQ-блоке внизу страницы в виде:
```
Номер телефона "НАЗВАНИЕ" ООО - 71 2270834; 71 2000056; 1347
```
Это единственный способ получить телефоны без Selenium/Playwright.

**Внешние ссылки:** сайты компаний скрыты за редиректом `/go/?u=HASH`. Резолвинг через HEAD-запрос или `title`-атрибут ссылки.

**Пагинация рубрик:** механизм не подтверждён — `?page=2` не работает стандартно (BLOCKER, задача 0.2).

**Кнопка "Скачать список":** на страницах рубрик есть иконка Excel-экспорта — если работает без авторизации, это предпочтительный способ (задача 0.3).
...
## Статус проекта и BLOCKER-задачи

Проект **не начат**. Перед написанием кода обязательна Фаза 0 — разведка:

| ID   | Задача                                              | Приоритет |
|------|-----------------------------------------------------|-----------|
| 0.1  | Проверить `robots.txt`                              | HIGH      |
| 0.2  | **[BLOCKER]** Установить механизм пагинации рубрик  | CRITICAL  |
| 0.3  | **[BLOCKER]** Проверить кнопку "Скачать список"     | CRITICAL  |
| 0.4  | Установить параметры POST-формы поиска              | MEDIUM    |
| 0.5  | Проверить стабильность FAQ-блока с телефонами       | HIGH      |
| 0.6  | Исследовать резолвинг `/go/?u=HASH`                 | MEDIUM    |
| 0.7  | Собрать таблицу ID всех городов/регионов            | MEDIUM    |

### Ветвление архитектуры по итогам разведки

- **Путь A** (предпочтительный): "Скачать список" работает без авторизации → `excel_downloader.py`
- **Путь B** (базовый): серверная пагинация (GET-параметр) → `requests` + `BeautifulSoup`
- **Путь C** (запасной): пагинация через JavaScript → весь стек меняется на `playwright`
